...
function mazdakdev_scripts()
{
	...
	wp_enqueue_script('mazdakdev-font-awesome', 'https://kit.fontawesome.com/ccc8febacc.js', array(), null, true);
	wp_enqueue_style('mazdakdev-style', get_stylesheet_uri(), array(), MAZDAKDEV_VERSION);
	wp_register_script('mazdakdev-script', get_template_directory_uri() . '/js/script.min.js', array(), MAZDAKDEV_VERSION, true);
 ...
 */
function mazdakdev_custom_comment_form_fields($fields)
{
...
add_filter('comment_form_default_fields', 'mazdakdev_custom_comment_form_fields');
...
add_filter('comment_form_field_comment', 'mazdakdev_custom_comment_field');
...
add_filter('comment_form_defaults', 'mazdakdev_custom_comment_form_defaults');
